 ...
 * 
 */
public class AccountTransferXuActivity extends Activity implements
		OnDismissListener {
	...
	public OnClickListener okAction = new OnClickListener() {
		...
		public void onClick(View v) {
			if (!mUser.getText().toString().equals("")
				...
				mCurrentDialog = ProgressDialog.show(
						AccountTransferXuActivity.this, null,
						getString(R.string.menu_waiting), true);
				mCurrentDialog
						.setOnDismissListener(AccountTransferXuActivity.this);
				mIsWaiting = true;
				Thread t = new Thread() {
					public void run() {
						Status = getResultXu(mOwner.getText().toString(),
								PrefUtil.GetStringPref(
										AccountTransferXuActivity.this,
										"password"), mNumber.getText()
										.toString(), mUser.getText().toString());
						if (Status.equals("1")) {
				...
				Toast.makeText(AccountTransferXuActivity.this,
						getString(R.string.transfer_xu_status),
						Toast.LENGTH_LONG).show();
			}
		}
	};

	public String getResultXu(String mOwner, String password, String mOwnerXu,
			String mUser) {
		String json = mEngine
				.Query_URL("http://16.test-kltn1.appspot.com/getUserFuntion.vn?flag=XuSendCustomer&content="
						+ mOwner
						+ "@"
						+ password
						+ "@"
	...
	public void onDismiss(DialogInterface dialog) {
		Toast.makeText(AccountTransferXuActivity.this, Status,
				Toast.LENGTH_LONG).show();
		if (flag = true) {
			mOwnerXu.setText(mEngine.GetXu(mOwner.getText().toString(),
					PrefUtil.GetStringPref(AccountTransferXuActivity.this,
							"password")));
			mNumber.setText("");
			mUser.setText("");
		}
