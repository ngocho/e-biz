...
public class TabBrowserActivity extends Activity implements OnClickListener,
	...
	public void LoadData() {
		mCurrentDialog = ProgressDialog.show(TabBrowserActivity.this, null,
	...
	public class BrowserAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			TextView name = (TextView) convertView
					.findViewById(R.id.item_browser_name);
	...
	public class BrowserItemAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (convertView == null)
				convertView = getLayoutInflater().inflate(
						R.layout.item_browser_get, null);
			ImageView image = (ImageView) convertView
					.findViewById(R.id.item_browser_get_image);
			BitmapManager.INSTANCE.loadBitmap(mEngine.URL_Image
					+ mItemsData.get(position).getImageurl(), image, 500, 500);
			TextView name = (TextView) convertView
					.findViewById(R.id.item_browser_get_name);
			TextView price = (TextView) convertView
					.findViewById(R.id.item_browser_get_buy);
			TextView pricepromotion = (TextView) convertView
					.findViewById(R.id.item_browser_get_buyend);
			TextView date = (TextView) convertView
					.findViewById(R.id.item_browser_get_date);
			TextView content = (TextView) convertView
					.findViewById(R.id.item_browser_get_content);
			...
			price.setPaintFlags(price.getPaintFlags()
					| Paint.STRIKE_THRU_TEXT_FLAG);
			pricepromotion.setText("$" + item.getBuyprice() + " "
					+ getString(R.string.vnd));
			TextView provider = (TextView) convertView
					.findViewById(R.id.item_browser_get_provider);
			...
			TextView buyer = (TextView) convertView
					.findViewById(R.id.item_browser_get_buyer);
	...
	private Button mOk;
	private String[] mData = { "Thực phẩm sơ chế", "Thức ăn nấu sẵn",
			"Rau xanh", "Trái cây", "Cửa hàng" };
	...
	public void onClick(View v) {
		...
		case R.id.tab_browser_back:
			if (!flag) {
				Intent menu = new Intent(TabBrowserActivity.this,
						MenuActivity.class);
		...
		case R.id.tab_browser_best:
			Intent best = new Intent(TabBrowserActivity.this,
					TabBestActivity.class);
			startActivity(best);
			finish();
			break;
		...
		case R.id.tab_browser_deal:
			Intent deals = new Intent(TabBrowserActivity.this,
					TabDealsActivity.class);
			startActivity(deals);
			finish();
			break;
		case R.id.tab_browser_home:
			Intent home = new Intent(TabBrowserActivity.this,
					MenuActivity.class);
		...
		case R.id.tab_browser_savesearch:
			if(flag){
				for(int i=0;i<mItemsData.size();i++){
					...
					mFavoriteEngine.put(new FavoriteItem(item.getId(), item.getName(), item.getIntroduction(), item.getPrice(), item.getBuyprice(), item.getImageurl(), item.getStartdate(), item.getStartdate(), item.getBuycount(), item.getBuycount(), item.getBuycount(), item.getBuycount(), item.getProvider()));
				}
			}
			break;
		}
